...
country = list(map(lambda x:x["name"], countries))
...
def contains_country(text):
    '''  
    Function used to filter text that contains the country name

    Parameters
    ----------
    arg1 : text
        text (string) that we want to filter

    Returns
    -------
    Boolean
        True if the text contains country name, else False
    '''
    for x in country:
...
def get_country_continent(company):
    '''  
    Function used to get the country and continent from a company

    Parameters
    ----------
    arg1 : company
        company name that we want get the continent and country
        
    Returns
    -------
    Tuple
        Tuple of continent and country of the company
    '''
    results = scrape(company + "company country")
    results = list(filter(lambda x: filter_link(x,company), results))
    #print(results)
    text = []
    for x in results:
      try:
        ...
        res = list(map(lambda x: contains_country(x), soup))
        res = list(filter(lambda x: x != "not found", res))
        text.extend(res)
      except:
        continue

    text2 = list(map(lambda x: (text.count(x),x), text))
    text2.sort(key=lambda x:x[0])
    final = text2[-1][1]
    idx = country.index(final)
    continent = countries[idx]["continent"]
    return continent, final

def get_logo(company):
    '''  
    Function used to get the logo from a company

    Parameters
    ----------
    arg1 : company
        company name that we want get the logo
